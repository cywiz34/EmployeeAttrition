...
for i in range(2):
    ...
    #for c, feature in sorted(zip(contributions[i],list(x_test)),key=lambda x: -abs(x[0])):

    for c in range(len(contributions[i])):

        data[i][0][list(x_test)[c]]=round(contributions[i][c][0],2)
        data[i][1][list(x_test)[c]]=round(contributions[i][c][1],2)
    print "-"*20
print data
'''
